...
export function diagnoseObservations(
  ...
  for (const [subject, list] of bySubject) {
    ...
    // 2. Failing scheduled run (raw failures, or a streaked heartbeat)
    const failures = list.filter(
      (o) => o.kind === "run-outcome" && o.attributes.status === "failed",
    );
    const streak = list.find(
      (o) =>
        o.kind === "schedule-heartbeat" &&
        typeof o.attributes.consecutiveFailures === "number" &&
        (o.attributes.consecutiveFailures as number) >=
          thresholds.maxConsecutiveFailures,
    );
    if (
      failures.length >= thresholds.maxConsecutiveFailures ||
      streak !== undefined
    ) {
 ...
 */
export function heldOutTasksFromHistory(
  changeKind: string,
  ...
  if (changeKind === "model-routing") {
    for (const o of evidence
      .filter((e) => e.kind === "model-usage")
      .slice(-50)) {
      tasks.push({
        id: `hist-${o.id}`,
        taskClass: String(o.attributes.taskClass ?? "unknown"),
        input: JSON.stringify({
          modelTier: o.attributes.modelTier ?? "unknown",
          costUsd: o.attributes.costUsd ?? 0,
          latencyMs: o.attributes.latencyMs ?? 0,
        }),
        assertions: ["replayed-historical-workload"],
      });
    }
  } else if (changeKind === "prompt-compaction") {
    for (const o of evidence
      .filter((e) => e.kind === "agent-trace")
      .slice(-50)) {
      tasks.push({
        id: `hist-${o.id}`,
        taskClass: "agent-trace",
        input: JSON.stringify({
          tokensUsed: Number(o.attributes.tokensUsed ?? 0),
          timeUsedSeconds: Number(o.attributes.timeUsedSeconds ?? 0),
        }),
        assertions: ["replayed-historical-workload"],
      });
    }
