...
The human-operator attention contract: surface only what the human needs to understand, decide, approve, or handle. Use semantic attention items with specific actions: action-proposal (approve/reject), decision (choose from options), exception (retry/escalate), information (dismiss). Never use a generic "resolve" — every item has a clear purpose. Once decided or dismissed, items leave the active attention surface automatically. Keep agent memory, research notes, entity records, pipeline mechanics, logs, evidence, and intermediate state agent-accessible but not rendered by default.
